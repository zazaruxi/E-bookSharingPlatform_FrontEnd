    ...
    <div class="column" >
      <q-card >
            <q-item
              ...
              class="row">
              <div class="column col-2 text-center" style="min-width: 200px">
                <q-img style="border-radius: 50%"
                       :ratio="1" width="180px" :src="'http://localhost:8081/user/img/'+commentInfo.userId"></q-img>
                {{commentInfo.userName}}
              </div>
              <q-separator class="q-mr-xl" vertical></q-separator>
              <div class="column ">
                <div class="q-pa-md col-10" style="font-size: 15px">
                  {{commentInfo.commentContent===undefined? commentInfo.replyContent:commentInfo.commentContent}}
                  <div>
                    <div class="row"  v-if="commentInfo.imgUrl!==null"  style="margin-left: 90px;margin-top: 70px" >
        <q-img class="q-ma-sm col-4"
               v-for="item2 in commentInfo.imgUrl"
               :key="commentInfo.id" :ratio="3/4"
               width="100px"
               loading="lazy"
               placeholder-src="data:image/png;base64,"
               :src="'http://localhost:8081/bookComment/img/'+commentInfo.id+'/'+item2">
        </q-img>
                  </div>
                  </div>
                </div>
                ...
                <div class="row col-2">
                  <div class="q-ma-sm text-grey-5">
                    {{commentInfo.updateTime}}

                  </div>

                  <q-btn color="secondary" clickable flat  icon="message">
                    <template v-slot:default>
                      <div style="size: 2px">{{commentInfo.replyCount}}</div>
                    </template>
                  </q-btn>
                </div>
              </div>
...
export default {
  ...
  setup(){
    ...
    const tp= ref(Router.currentRoute.value.query.type)
    ...
    onMounted(()=>{
      try{
        ...
        if ('WebSocket' in window) {
          console.log(tp.value+cid.value)
          websocket = new WebSocket('ws://localhost:8081/push/websocket/'+tp.value+cid.value,[getToken()])
          // alert('连接浏览器')
          initWebSocket()
        } else {
          alert('Not support')
    ...
    const  setOnmessageMessage =(event)=> {
      ...
      comment.commentInfo.replyCount+=1
      move.value+=1
    }
    const reflash = (val) => {
      ...
      while (newCol.li.length!==0){
        reply.repList.list.unshift(newCol.li.pop())
      }
      loading.value=false
    }
...
const init=()=>{
  ...
  newCol.li=[]
  api.get(
    tp.value==='com'? '/bookComment/' + cid.value:'/bookCommentReply/' + cid.value
  ).then(
    res => {
      if (res.data.code === 200) {
        console.log(res.data)
        comment.commentInfo = res.data.data
      }
    })
  api.get(
    'bookCommentReply/comment/'+tp.value+cid.value,
    {
      params:{
      ...
      writeComment(){
        ...
        q.dialog({
          ...
          componentProps: {
            ...
            tp:tp.value,
            comt:false,
            bookId:Router.currentRoute.value.query.bookId
          },
        }).onOk(() => {
          click.value=false
        }).onCancel(() => {
...
methods:{
  lis(e){
    ...
    if (scrollTop != 0 && bottomOfWindow && !this.$q.loading.isActive&&!this.finished) {
      this.$api.get(
        'bookCommentReply/comment/'+this.tp+this.cid,
        {
          params: {
